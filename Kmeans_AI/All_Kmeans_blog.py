...
while running:
	...
	if 55 < mouse_x < 745 and 55 < mouse_y < 545:
		position_mouse_x = mouse_x - 55
		position_mouse_y = mouse_y - 55
		text_mouse = font_small.render("("+str(position_mouse_x)+","+str(position_mouse_y)+")",True,BLACK) 
	...
	for i in range(len(points)):
		...
		if labels == []:
			pygame.draw.circle(screen,BACKGROUND_PANEL,(points[i][0]+55,points[i][1]+55),4)	
		else:
	...
	if clusters != [] and labels != []:
		for i in range(len(points)):
			error += distance(clusters[labels[i]],points[i])
